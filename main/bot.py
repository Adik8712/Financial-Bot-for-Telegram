...
def chart_source(message):
    if message.text.lower() == 'отмена':
    ...
    elif message.text == "График расходов":
        ...
        if r.status_code == 200:
            ...
            bot.send_message(message.chat.id, "График повреждён!", reply_markup=menu_button())
        
        return
    
    elif message.text == "График доходов":
        chart_income_photo_url = chart.user_chart_income(message.chat.id)
        r = requests.get(settings.MY_DOMAIN + chart_income_photo_url)
        if r.status_code == 200:
            bot.send_photo(message.chat.id, r.content, reply_markup=menu_button())
        else:
            bot.send_message(message.chat.id, "График доходов повреждён!", reply_markup=menu_button())
