...
class TelegramUser(models.Model):
    ...
    chart = models.FileField(
        ...
        blank=True,
        null=True,
        verbose_name="График расходов",
        help_text="Файл с графиком расходов пользователя.",
    )

    chart_income = models.FileField(
        upload_to="chart_income/",
        blank=True,
        null=True,
        verbose_name="График доходов",
        help_text="Файл с графиком доходов пользователя.",
    )
...
class TelegramUserIncome(models.Model):
    ...
    name = models.CharField(
        ...
        help_text = "От куда пришёл доход.",
    )
...
class TelegramExpense(models.Model):
    user = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="Пользователь Telegram",
        help_text="Пользователь Telegram, которому принадлежит расход.",
    )
    name = models.CharField(
        max_length=100,
        verbose_name = "Название расхода",
        help_text = "Куда пришёл расход.",
    )
    amount = models.BigIntegerField(
        default = 0,
        verbose_name="Сумма",
